...
function clearDefTheme() {
    store.setItem("theme", "neondark");
    changeTheme(
        `./themes/${themeLookup[store.getItem("theme")]}.css`,
        store.getItem("theme"),
    );
}
...
document.getElementById("close_button").addEventListener("click", (event) => {
    console.log(event);
    console.log(
        `Current Service: ${document.getElementById("services").value}`,
    );
    console.log(`showTTY: ${document.getElementById("TTY").checked}`);
    console.log(
        `useVideoThumbnails: ${document.getElementById("uVT").checked}`,
    );
